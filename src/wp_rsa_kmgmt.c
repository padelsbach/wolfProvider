    ...
    int priv)
{
    ...
    if (ok) {
        int cnt = wp_params_count(params);
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            int index = -1;
            for (unsigned j = 0; j < ARRAY_SIZE(wp_rsa_param_key); j++) {
            ...
            if (index < 0) {
                WOLFPROV_MSG(WP_LOG_PK, "Unknown param %s", p->key);
                ok = 0;
                break;
            }
            ...
            if (ok) {
                mp_int* mp = (mp_int*)(((byte*)&rsa->key) + wp_rsa_offset[index]);
