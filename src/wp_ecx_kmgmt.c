    ...
    void* cbArg)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_KE, "wp_ecx_export");
...
static int wp_ecx_gen_set_params(wp_EcxGenCtx* ctx, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_KE, "wp_ecx_gen_set_params");
...
static wp_Ecx* wp_ecx_gen(wp_EcxGenCtx* ctx, OSSL_CALLBACK* osslcb, void* cbarg)
{
    ...
    if ((ecx != NULL) && keyPair) {
        int rc = (*ctx->data->makeKey)(&ctx->rng, ctx->data->len,
            (void*)&ecx->key);
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "makeKey", rc);
            wp_ecx_free(ecx);
            ecx = NULL;
        }
        else {
    ...
    const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_KE, "wp_ecx_enc_dec_set_ctx_params");
    ...
    OSSL_CALLBACK* dataCb, void* dataCbArg)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_KE, "wp_ecx_dec_send_params");
    ...
    OSSL_PASSPHRASE_CALLBACK* pwCb, void* pwCbArg)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_KE, "wp_ecx_decode");
    ...
    OSSL_PASSPHRASE_CALLBACK *pwCb, void *pwCbArg)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_KE, "wp_ecx_encode");
